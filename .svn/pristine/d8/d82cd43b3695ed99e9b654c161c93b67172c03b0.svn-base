...
void AST::swap(AST& rhs) {
    ...
    std::list<AST*>::iterator temp = child.begin();

    while (temp != child.end()) {
        tempChild.push_back(*temp);
        ++temp;
    }*/
...
void AST::mainHeader(const std::vector<std::string>& profileName,
    ...
    std::string baseString = "#include \"profile.hpp\"\n";
    ...
    std::list<AST*>::const_iterator it = child.begin(); 
        ...
        child.insert(it, new AST(token, baseString));

        std::vector<std::string>::const_iterator profileIt = profileName.begin();
        std::vector<std::string>::const_iterator fileIt = fileName.begin();


        while(profileIt != profileName.end()){
             while(fileIt != fileName.end()){
           
            /*for (std::vector<std::string>::const_iterator str = profileName.begin(); str != profileName.end(); ++str) {
                fileString += *str;
            }*/
            std::string profileCpp = (*profileIt);
            //std::string fileString = "profile " + profileCpp;
            //child.insert(it, new AST(token, fileString));

            /*std::string newFileString = "";
            for (std::vector<std::string>::const_iterator newStr = fileName.begin(); newStr != fileName.end(); ++newStr) {
                newFileString += *newStr;
            }*/
            std::string fileCpp = (*fileIt);
            
           fileCpp += "\");\n";

           
           std::string inputString = "profile " + profileCpp + "(\"" + fileCpp;
            child.insert(it, new AST(token, inputString));
            ++profileIt;
            ++fileIt;
            }
    ...
    while(i != profileName.end()) {     
        ...
        std::string fileCpp = (*i); //The original profile name
...
void AST::fileHeader(const std::string& profileName) {
    ...
    std::string newFileString = "extern profile " + profileName + ";\n";

    child.insert(it, new AST(token, newFileString));
    ...
    while(it != child.end()) {
        if(((*it)->tag == "function")){
            if((*it)->getChild("name")->getName() == "main"){
                AST *blockFinder = (*it)->getChild("block")->getChild("block_content");
                std::string outputResult;
                for (std::vector<std::string>::const_iterator str = profileName.begin(); str != profileName.end(); ++str) {
                    outputResult = outputResult + ("std::cout << " + (*str) + " << std::endl;\n");
                }
                blockFinder->child.insert(----blockFinder->child.end(), new AST(token, outputResult));
            }
        }
        ++it;
    }
...
void AST::functionCount(const std::string& profileName) {
    ...
    while(it != child.end()) {
        if(((*it)->tag == "function")){
            std::string name = (*it)->getChild("name")->getName();
            AST *blockFinder = (*it)->getChild("block")->getChild("block_content");
            std::string count = " " + profileName + ".count(__LINE__, \"" + name + "\");";
            blockFinder->child.insert(blockFinder->child.begin(), new AST(token, count));
        }
        ++it;
    }
...
void AST::lineCount(const std::string& profileName) {
    ...
    /*std::string insert= " " + profileName + ".count(__LINE__);";
    for(std::list<AST*>::const_iterator i = child.begin(); i != child.end(); ++i) {
        if((*i)->tag == "function") {
            AST *blockFinder = (*i)->getChild("block")->getChild("block_content");
            std::list<AST*>::iterator blockIt = blockFinder->child.begin();
            
            while(blockIt != blockFinder->child.end()) {
                if((*blockIt)->tag == "expr_stmt") {
                    ++blockIt;
                    child.insert(blockIt, new AST(token, insert));
                }
                else{
                    if(isStopTag((*blockIt)->tag) == false){
                        
                        (*blockIt)->lineCount(profileName);
                    }
                }
                ++blockIt;
            }
    ...
    std::list<AST*>::iterator it = child.begin();
        ...
        while(it != child.end()) {
            if((*it)->tag == "expr_stmt") {
                ++it;
                child.insert(it, new AST(token, insert));
            }
            else {
